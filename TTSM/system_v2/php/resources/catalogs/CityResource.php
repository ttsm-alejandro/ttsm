...
function importPhpFiles(){
    ...
    require "../../util/Util.php";
    //models
    require "../../models/CityModel.php";
    //services
    require "../../services/CityService.php";
}
...
function returnDataDELETE( $link ){
    $id = $_GET[ "id" ];
    echo CityService::delete( $link , $id );
}
...
function returnDataPOST( $link ){
    ...
    $newElement = new CityModel(
            $datos["id"],
            $datos["idState"],
            $datos["name"]
            );
    $returnInfo = CityService::save( $link , $newElement );
    echo $returnInfo;
}
...
function returnDataGET( $link ){
    if( isset( $_GET[ "id" ] ) ){
        $id = $_GET[ "id" ];
        echo json_encode( CityService::getById( $link , $id ) );
    }else{
        echo json_encode( CityService::getAll( $link ) );
    }
}
