...
function importPhpFiles(){
    ...
    require "../../util/Util.php";
    //models
    require "../../models/SoftwareModel.php";
    //services
    require "../../services/SoftwareService.php";
}
...
function returnDataDELETE( $link ){
    $id = $_GET[ "id" ];
    echo SoftwareService::delete( $link , $id );
}
...
function returnDataPOST( $link ){
    ...
    $newElement = new SoftwareModel(
            $datos["id"],
            $datos["name"],
            $datos["comment"]
            );
    $returnInfo = SoftwareService::save( $link , $newElement );
    echo $returnInfo;
}
...
function returnDataGET( $link ){
    if( isset( $_GET[ "id" ] ) ){
        $id = $_GET[ "id" ];
        echo json_encode( SoftwareService::getById( $link , $id ) );
    }else{
        echo json_encode( SoftwareService::getAll( $link ) );
    }
}
