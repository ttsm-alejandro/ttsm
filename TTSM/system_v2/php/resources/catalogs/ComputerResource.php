...
function importPhpFiles(){
    ...
    //services
    require "../../services/ComputerService.php";
}
...
function returnDataDELETE( $link ){
    $id = $_GET[ "id" ];
    echo ComputerService::delete( $link , $id );
}
...
function returnDataGET( $link ){
    if( isset( $_GET[ "id" ] ) ){
        $id = $_GET[ "id" ];
        echo json_encode( ComputerService::getById( $link , $id ) );
    }else{
        echo json_encode( ComputerService::getAll( $link ) );
    }
}
