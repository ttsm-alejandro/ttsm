...
function returnDataDELETE( $link ){
    $id = $_GET[ "id" ];
    echo DepartmentService::delete( $link , $id );
}
...
function returnDataPOST( $link ){
    ...
    $newElement = new DepartmentModel(
            $datos["id"],
            $datos["idCompany"],
            $datos["idPlant"],
            $datos["idPerson"],
    ...
    $returnInfo = DepartmentService::save( $link , $newElement );
    echo $returnInfo;
}
...
function returnDataGET( $link ){
    if( isset( $_GET[ "id" ] ) ){
        $id = $_GET[ "id" ];
        echo json_encode( DepartmentService::getById( $link , $id ) );
    }else{
        echo json_encode( DepartmentService::getAll( $link ) );
    }
}
