...
function importPhpFiles(){
    ...
    require "../../util/Util.php";
    //models
    require "../../models/MachineTypeModel.php";
    //services
    require "../../services/MachineTypeService.php";
}
...
function returnDataDELETE( $link ){
    $id = $_GET[ "id" ];
    echo MachineTypeService::delete( $link , $id );
}
...
function returnDataPOST( $link ){
    ...
    $newElement = new MachineTypeModel(
            $datos["id"],
            $datos["name"],
            $datos["comment"]
            );
    $returnInfo = MachineTypeService::save( $link , $newElement );
    echo $returnInfo;
}
...
function returnDataGET( $link ){
    if( isset( $_GET[ "id" ] ) ){
        $id = $_GET[ "id" ];
        echo json_encode( MachineTypeService::getById( $link , $id ) );
    }else{
        echo json_encode( MachineTypeService::getAll( $link ) );
    }
}
