...
function importPhpFiles(){
    ...
    require "../../util/Util.php";
    //models
    require "../../models/ControlPaseModel.php";
    //services
    require "../../services/ControlPaseService.php";
}
...
function returnDataDELETE( $link ){
    $id = $_GET[ "id" ];
    echo ControlPaseService::delete( $link , $id );
}
...
function returnDataPOST( $link ){
    ...
    $newElement = new ControlPaseModel(
            $datos["id"],
            $datos["date"],
            $datos["destiny"],
            $datos["user"],
    ...
    $returnInfo = ControlPaseService::save( $link , $newElement );
    echo $returnInfo;
}
...
function returnDataGET( $link ){
    if( isset( $_GET[ "id" ] ) ){
        $id = $_GET[ "id" ];
        echo json_encode( ControlPaseService::getById( $link , $id ) );
    }else{
        echo json_encode( ControlPaseService::getAll( $link ) );
    }
}
