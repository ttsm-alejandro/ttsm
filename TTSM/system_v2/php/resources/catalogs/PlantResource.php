...
function importPhpFiles(){
    ...
    //services
    require "../../services/PlantService.php";
}
...
function returnDataDELETE( $link ){
    $id = $_GET[ "id" ];
    echo PlantService::delete( $link , $id );
}
...
function returnDataPOST( $link ){
    ...
    $returnInfo = PlantService::save( $link , $newElement );
    echo $returnInfo;
}
...
function returnDataGET( $link ){
    if( isset( $_GET[ "id" ] ) ){
        $id = $_GET[ "id" ];
        echo json_encode( PlantService::getById( $link , $id ) );
    }else{
        echo json_encode( PlantService::getAll( $link ) );
    }
}
