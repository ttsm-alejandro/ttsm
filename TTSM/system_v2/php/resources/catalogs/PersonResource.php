...
function returnDataDELETE( $link ){
    $id = $_GET[ "id" ];
    echo PersonService::delete( $link , $id );
}
...
function returnDataPOST( $link ){
    ...
    $returnInfo = PersonService::save( $link , $newElement );
    echo $returnInfo;
}
...
function returnDataGET( $link ){
    if( isset( $_GET[ "id" ] ) ){
        $id = $_GET[ "id" ];
        echo json_encode( PersonService::getById( $link , $id ) );
    }else{
        echo json_encode( PersonService::getAll( $link ) );
    }
}
