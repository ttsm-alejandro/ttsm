...
miApp.controller( 'machineTypeCtrl'  ,['$scope' , '$http' , '$window' , function( $scope , $http , $window ){
    /**** variables ****/
    //URL
    var machineTypeServiceUrl = serviceUrl + "php/resources/catalogs/MachineTypeResource.php";
    $scope.relativeUrl = "../../";
    ...
    $scope.getData = function(){
        ...
        $http({
            url: machineTypeServiceUrl + "?user=" + $scope.user + "&token=" + $scope.token,
            method: "GET"
        })
    ...
    $scope.getDataById = function( id ){
        $('#myLoadingModal').modal('hide'); 
        $http({
            url: machineTypeServiceUrl + "?user=" + $scope.user + "&token=" + $scope.token + "&id=" + id,
            method: "GET"
        })
        .then(function(response) {
            if(response.data == "ACCESS DENIED" ){
    ...
    $scope.deleteRow = function(){
        if( $scope.details.id == "--" ){
            swal( { text :  "Select Machine Type" , icon : "error" } );
        }else{
            swal({
    ...
    $scope.deleteInDB = function(){
        $('#myLoadingModal').modal('show'); 
        $http({
            url: machineTypeServiceUrl + "?user=" + $scope.user + "&token=" + $scope.token + "&id=" + $scope.details.id + "&delete=ok",
            method: "DELETE"
        })
        .then(function(response) {
            $scope.cleanDetails();
    ...
    $scope.isDetailsDataOk = function(){
        ...
        if( !$scope.isDataCombinationOk() ){ returnData = false; errorText += "Already exist a Machine Type with the same Name, "; }
